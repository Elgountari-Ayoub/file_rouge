<template>
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <VueDatePicker v-model="date" inline auto-apply="true" class="col-span-1" />
        <div class="relative grid col-span-2 gap-4 text-center place-items-start">
            <ul class="grid grid-cols-2 gap-4 md:grid-cols-4">
                <li v-for="item in appointments" :key="item.start_time" class="p-2 text-center" @click="item.client_id ? null :  updateSelectedDate(item.start_time)"  :class="{'bg-red-500' : item.client_id, 'bg-teal-100' : !item.client_id}">
                
                <!-- <li v-for="item in appointments" :key="item.start_time" class="p-2 text-center bg-gray-100 rounded"> -->
                    {{ item.start_time }}
                </li>
            </ul>
            <button type="submit"
